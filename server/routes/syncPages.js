...
router.post("/item", requireWriteAccess, async (req, res) => {
  ...
  try {
    ...
    store.createSyncJob({ id: jobId, accountId, mode: "pages-item", total: ids.length, wxrksProjectUUID: project.uuid, orgUnitUUID, targetLocales });
      ...
      pages: pages.map((p) => {
        ...
        settings.targetLocales.forEach((locale) => {
          const { status, error } = store.computeLocaleStatus({
            delivery: deliveryStatus[p.id]?.[webflow.normalizeLocaleTag(locale)],
